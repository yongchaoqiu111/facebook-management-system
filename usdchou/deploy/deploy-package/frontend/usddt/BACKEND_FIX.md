 ...
 * 后端修复补丁
 * 
 * 文件：d:\weibo\usdchou\services\chainRedPacketService.js
 * 
 * 问题：缺少 getChainGroupInfo 和 joinChainGroup 方法
 */

// ============ 在 getTopClaimers 函数之后，module.exports 之前添加 ============
...
async function joinChainGroup(groupId, userId) {
  ...
  try {
    ...
    const redPacket = new RedPacket({
      ...
      totalAmount: firstRedPacketAmount * 100,
...
// ============ 修改 module.exports 部分 ============

// 找到文件末尾的 module.exports，改为：
