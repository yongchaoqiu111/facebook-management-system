...
function loadPrompts() {
    const promptsFile = path_1.default.resolve('D:\\weibo\\data\\prompts.json');
    try {
        if (fs_1.default.existsSync(promptsFile)) {
            const content = fs_1.default.readFileSync(promptsFile, 'utf8');
...
function saveNews(news, keyword) {
    const newsPath = path_1.default.join(__dirname, '../../user-config/assets/news/1.txt');
    // 确保目录存在
    const newsDir = path_1.default.dirname(newsPath);
...
function savePost(post) {
    ...
    const postPath = path_1.default.join(postDir, '2.txt');
