...
exports.commentInterceptSkill = commentInterceptSkill;
...
// 获取当前文件目录
const __dirname = path_1.default.dirname(process.cwd());
// 读取文本文件内容
...
// 检查评论是否包含目标关键词
function containsKeyword(comment, keywords) {
    const commentLower = comment.toLowerCase();
    return keywords.some(keyword => commentLower.includes(keyword.toLowerCase()));
}
// 模拟Facebook评论回复
async function replyToComment(commentId, replyContent) {
    try {
...
async function commentInterceptSkill(input) {
    log('开始评论区截留任务');
    try {
        // 定义文件路径
        const searchKeywordsFile = path_1.default.join(__dirname, '../../user-config/assets/pinglunci/ss.txt');
        const targetKeywordsFile = path_1.default.join(__dirname, '../../user-config/assets/pinglunci/pinlunci.txt');
        const replyContentsFile = path_1.default.join(__dirname, '../../user-config/assets/pinglunci/huifu.txt');
        // 读取关键词和回复内容
