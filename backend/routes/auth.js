...
const UserFriends = require('../models/UserFriends'); // 🆕 好友模型
  ...
  try {
    ...
    try {
      ...
      jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '7d' }, (err, token) => {
        ...
        const balances = {
          btcBalance: parseFloat((cryptoHoldings.get('BTC') || 0).toFixed(8)),
          ethBalance: parseFloat((cryptoHoldings.get('ETH') || 0).toFixed(8)),
          bnbBalance: parseFloat((cryptoHoldings.get('BNB') || 0).toFixed(8)),
          solBalance: parseFloat((cryptoHoldings.get('SOL') || 0).toFixed(8)),
          xrpBalance: parseFloat((cryptoHoldings.get('XRP') || 0).toFixed(8))
        };

        // 🆕 获取好友ID数组（新用户为空）
        UserFriends.findOne({ userId: user.userId }).then(userFriends => {
          ...
          console.error('Failed to get friend list for new user:', err);
          // 即使失败也返回基本数据
          sendSuccess(res, { 
            token, 
            user: { 
