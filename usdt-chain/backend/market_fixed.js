    ...
    handleWebSocketMessage(data) {
      if (data.event === 'bts:subscription_succeeded') {
        ...
        this.coins.forEach(coin =>{
          ...
          // 🔥 修复：不重新计算涨跌幅，保持 fetchCoinList 时的初始值
          // const changePercent = ((priceVariation - prevPrice) / prevPrice * 100)
          // coin.change = changePercent.toFixed(4)
          console.log(`💰 ${coin.symbol}: ${prevPrice} → ${priceVariation.toFixed(2)} (涨跌幅: ${coin.change}%)`)
        })
        ...
        if (this.selectedCoin) {
          ...
          // 🔥 修复：不重新计算涨跌幅，保持 fetchCoinList 时的初始值
          // const changePercent = ((priceVariation - prevPrice) / prevPrice * 100)
          // this.selectedCoin.change = changePercent.toFixed(4)
          console.log(`🎯 选中币种更新: ${this.selectedCoin.symbol} ${prevPrice} → ${priceVariation.toFixed(2)} (涨跌幅: ${this.selectedCoin.change}%)`)
