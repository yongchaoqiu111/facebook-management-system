            ...
            this.page.on('request', (request) => {
                ...
                if (url.includes('kuaishou.com') || url.includes('ksapisrv.com')) {
                    ...
                    params.forEach((value, key) => {
                        if (['userId', 'user_id', 'userIdStr', 'userIdStr', 'profile', 'user', 'uid', 'user_id'].includes(key)) {
                    ...
                    try {
                        ...
                        for (const selector of selectors) {
                            ...
                            for (const item of elements) {
                                ...
                                const commentElement = item.querySelector('.comment');
                                
                                if (usernameElement && commentElement) {
                                    // 快手弹幕结构
                                    nickname = usernameElement.innerText.trim();
                                    content = commentElement.innerText.trim();
                                    
                                    // 移除用户名后面的冒号
                                    if (nickname.endsWith('：') || nickname.endsWith(':')) {
                                        nickname = nickname.slice(0, -1).trim();
                                    }
                                    
                                    if (!nickname || !content) continue;
                                    console.log(`找到快手弹幕: ${nickname}: ${content}`);
                                } else {
                                    // 抖音兼容模式
