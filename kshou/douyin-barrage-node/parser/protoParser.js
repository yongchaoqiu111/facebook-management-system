...
class ProtoParser {
    ...
    async loadProtoFiles() {
        try {
            ...
            if (fs.existsSync(protoDir)) {
                ...
                for (const file of protoFiles) {
                    ...
                    if (!this.root) {
                        ...
                        // 合并多个Proto文件
                        if (root.nested && typeof root.nested.forEach === 'function') {
            ...
            if (this.root) {
                // 获取所有消息类型
                if (this.root.nested && typeof this.root.nested.forEach === 'function') {
                    ...
                    console.warn('Proto文件结构不支持，使用默认解析方式');
                }
            } else {
    ...
    // 快手特定的消息解析
    parseKuaishouMessage(data) {
        try {
            // 快手的WebSocket消息格式通常是：[操作码][数据长度][数据]
            if (data.length < 8) {
                return null;
            }
