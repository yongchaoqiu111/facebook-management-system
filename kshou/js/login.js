...
class DouyinDanmakuAutomation {
    ...
    async start(roomUrl) {
        try {
            ...
            // 存储用户ID和用户主页URL的映射
            this.userIdToUrlMap = new Map();
            
            this.page.on('request', async (request) => {
                const url = request.url();
                const method = request.method();
                
                // 记录所有API请求，重点关注用户和弹幕相关接口
                if (url.includes('kuaishou.com') || url.includes('ksapisrv.com') || url.includes('kwai.com') || url.includes('kwaicdn.com')) {
                    console.log(`[API] ${method} 请求: ${url}`);
                    ...
                    if (method === 'POST') {
                        try {
                            ...
                            if (postData) {
                                console.log(`[API] POST请求体: ${postData}`);
                                
                                // 尝试解析JSON请求体
                                try {
                                    const data = JSON.parse(postData);
                                    console.log(`[API] 解析POST数据:`, JSON.stringify(data, null, 2));
                                    
                                    // 查找用户ID相关字段
                                    const userIdFields = ['userId', 'user_id', 'uid', 'principalId', 'authorId', 'anchorId', 'userIdStr'];
                                    for (const field of userIdFields) {
                                        if (data[field]) {
                                            const userId = data[field];
                                            const userUrl = `https://live.kuaishou.com/profile/${userId}`;
                                            console.log(`[API] 从POST请求体提取到用户ID: ${userId}`);
                                            console.log(`[API] 用户主页URL: ${userUrl}`);
                                            this.userIdToUrlMap.set(userId, userUrl);
                                        }
                                    }
                                } catch (e) {
                                    console.log(`[API] POST请求体不是JSON格式`);
                                }
                            }
                        } catch (e) {
                            console.log(`[API] 读取POST请求体失败:`, e.message);
                        }
                    }
                    
                    // 重点捕获用户信息API（关键！）
                    if (url.includes('/live_api/baseuser/userinfo/byid')) {
                        const principalId = params.get('principalId');
                        if (principalId) {
                            const userUrl = `https://live.kuaishou.com/profile/${principalId}`;
                            console.log(`✅ [API] 提取到用户ID: ${principalId}`);
                            console.log(`✅ [API] 用户主页URL: ${userUrl}`);
                            this.userIdToUrlMap.set(principalId, userUrl);
                            
                            // 将用户信息更新到页面
                            await this.page.evaluate((principalId, userUrl) => {
                                window._USER_ID_MAP[principalId] = userUrl;
                                console.log(`✅ 用户信息已更新: ${principalId} -> ${userUrl}`);
                            }, principalId, userUrl);
                        }
                    } else if (url.includes('/profile/')) {
                        // 捕获用户主页访问
                        const profileMatch = url.match(/\/profile\/([a-zA-Z0-9]+)/);
                        if (profileMatch) {
                            const userId = profileMatch[1];
                            const userUrl = `https://live.kuaishou.com/profile/${userId}`;
                            console.log(`[API] 捕获用户主页访问: ${userUrl}`);
                            this.userIdToUrlMap.set(userId, userUrl);
                        }
                    } else if (url.includes('/u/')) {
                        // 捕获用户主页访问（另一种格式）
                        const profileMatch = url.match(/\/u\/([a-zA-Z0-9]+)/);
                        if (profileMatch) {
                            const userId = profileMatch[1];
                            const userUrl = `https://live.kuaishou.com/profile/${userId}`;
                            console.log(`[API] 捕获用户主页访问: ${userUrl}`);
                    ...
                    } else if (url.includes('user') && url.includes('info')) {
                        // 捕获其他用户信息API
                        console.log(`[API] 可能的用户信息API: ${url}`);
                    } else if (url.includes('danmaku') || url.includes('comment') || url.includes('chat')) {
                        // 捕获弹幕相关API
                        console.log(`[API] 弹幕相关API: ${url}`);
                    } else if (url.includes('live_api')) {
                        // 捕获直播相关API
                        console.log(`[API] 直播API: ${url}`);
                    }
                    
                    // 查找用户相关参数
                    params.forEach((value, key) => {
                        if (['userId', 'user_id', 'userIdStr', 'userIdStr', 'profile', 'user', 'uid', 'user_id', 'principalId', 'authorId', 'anchorId', 'liveId'].includes(key)) {
                            console.log(`[API] 用户参数: ${key}=${value}`);
                        }
                    });
                }
            ...
            this.page.on('response', async (response) => {
                ...
                // 分析用户相关的响应
                if (url.includes('kuaishou.com') || url.includes('ksapisrv.com')) {
                        ...
                        try {
                            ...
                            if (text.includes('user') && text.includes('profile')) {
                                ...
                                try {
                                    ...
                                    // 递归查找用户ID
                                    function findUserId(obj, path = '') {
                                        if (!obj || typeof obj !== 'object') return null;
                                        
                                        for (const field of userIdFields) {
                                            if (obj[field]) {
                                                console.log(`[API] 在 ${path}.${field} 找到用户ID: ${obj[field]}`);
                                                return obj[field];
                                            }
                                        }
                                        ...
                                        for (const key in obj) {
                                            if (obj[key] && typeof obj[key] === 'object') {
                                                const result = findUserId(obj[key], path ? `${path}.${key}` : key);
                                    ...
                                    if (userId) {
                                        const userUrl = `https://live.kuaishou.com/profile/${userId}`;
                                        console.log(`[API] 提取到用户ID: ${userId}`);
                                        console.log(`[API] 用户主页URL: ${userUrl}`);
                                        this.userIdToUrlMap.set(userId, userUrl);
                                        
                                        // 将用户ID和URL发送到页面，更新用户信息
                                        await this.page.evaluate((userId, userUrl) => {
                                            window._USER_ID_MAP[userId] = userUrl;
                                            console.log(`✅ 用户信息已更新: ${userId} -> ${userUrl}`);
                                        }, userId, userUrl);
                                    }
                                } catch (e) {
                                    console.log(`[API] 响应不是JSON格式，显示部分内容:`, text.substring(0, 500));
                                }
                            }
                        } catch (e) {
            ...
            // 注入JavaScript来捕获弹幕，传入关键词和用户ID映射
            await this.page.evaluate((keywords, userIdMap) => {
                window._KEYWORDS = keywords || [];
                window._USER_ID_MAP = userIdMap || {};
                
                // 定义发送弹幕数据的函数（带关键词过滤）
                window.sendDanmaku = function(danmaku) {
                    // 只过滤系统消息（用户URL为空字符串是最大特征）
                    if (!danmaku.user.url || danmaku.user.url === '') {
                        console.log(`🔍 过滤：系统消息（userUrl为空） - ${danmaku.content}`);
                        return;
                    }
                    
                    // 过滤纯粹的"为主播点赞了"消息（不包含其他内容）
                    if (danmaku.content.trim() === '为主播点赞了') {
                        console.log(`🔍 过滤：纯粹的点赞消息 - ${danmaku.content}`);
                        return;
                    }
                    
                    // 基于userUrl去重：同一个用户只记录一条弹幕
                    if (!window.userUrlCache) {
                        window.userUrlCache = new Set();
                    }
                    
                    if (window.userUrlCache.has(danmaku.user.url)) {
                        console.log(`🔍 过滤：同一用户重复弹幕 - ${danmaku.content}`);
                        return;
                    }
                    ...
                    if (window._KEYWORDS.length > 0) {
                        const shouldProcess = window._KEYWORDS.some(keyword => 
                        ...
                        for (const selector of selectors) {
                            ...
                            for (const element of elements) {
                                ...
                                try {
                                    ...
                                    if (usernameElement) {
                                        // 检查用户名元素本身是否是链接
                                        if (usernameElement.tagName === 'A' && usernameElement.href) {
                                            userUrl = usernameElement.href;
                                            if (userUrl.startsWith('//')) {
                                                userUrl = 'https:' + userUrl;
                                            }
                                            console.log(`从用户名元素找到用户主页链接: ${userUrl}`);
                                        } else {
                                            // 检查用户名元素的父元素和兄弟元素
                                            ...
                                            while (parent && depth > 0) {
                                                ...
                                                for (const link of links) {
                                                    ...
                                                    if (href && (href.includes('kuaishou.com/u/') || href.includes('kuaishou.cn/u/') || href.includes('/u/'))) {
                                                        userUrl = href;
                                                        if (userUrl.startsWith('//')) {
                                                            userUrl = 'https:' + userUrl;
                                                        }
                                                        console.log(`从父元素找到用户主页链接: ${userUrl}`);
                                                        break;
                                                    }
                                                }
                                                if (userUrl) break;
                                    ...
                                    // 策略4：从用户名提取用户ID（快手）- 暂时禁用纯数字提取
                                    if (!userUrl && nickname) {
                                        // 匹配"快手用户1718877819364"格式的用户名
                                        const userIdMatch = nickname.match(/快手用户(\d+)/);
                                        if (userIdMatch) {
                                            const userId = userIdMatch[1];
                                            console.log(`从用户名提取到纯数字ID: ${userId} - 不使用纯数字作为用户ID`);
                                        }
                                    }
                                    
                                    // 策略5：尝试点击用户名打开弹出框，触发API请求（关键步骤）
                                    // 强制点击用户名，不依赖userUrl是否为空
                                    try {
                                        const usernameElement = element.querySelector('.username');
                                        if (usernameElement) {
                                            const actualNickname = usernameElement.textContent.trim().replace(/：$/, '');
                                            console.log(`✅ 强制点击用户名"${actualNickname}"来触发用户信息API请求...`);
                                            
                                            // 记录当前弹幕信息，用于后续关联
                                            window.currentDanmakuInfo = {
                                                nickname: actualNickname,
                                                content: content,
                                                timestamp: Date.now()
                                            };
                                            console.log(`✅ 已记录弹幕信息:`, window.currentDanmakuInfo);
                                            
                                            // 尝试点击用户名打开弹出框
                                            usernameElement.click();
                                            console.log('✅ 已点击用户名，触发用户信息API请求');
                                        }
                                    } catch (error) {
                                        console.log('点击用户名失败:', error.message);
                                    }
                                    
                                    // 策略6：从HTML属性查找用户ID（类似3xwzwqgc5kes3ai格式）
                                    if (!userUrl) {
                                        // 检查元素的所有属性
                                        for (let i = 0; i< element.attributes.length; i++) {
                                            const attr = element.attributes[i];
                                            const attrValue = attr.value;
                                            // 匹配字母数字混合的用户ID格式（如3xwzwqgc5kes3ai）
                                            const userIdMatch = attrValue.match(/([a-zA-Z0-9]{16,20})/);
                                            if (userIdMatch) {
                                                const userId = userIdMatch[1];
                                                userUrl = `https://live.kuaishou.com/profile/${userId}`;
                                                console.log(`从属性 ${attr.name} 提取到用户ID: ${userId}`);
                                                break;
                                            }
                                        }
                                    }
                                    
                                    // 策略5：抖音兼容 - 尝试从头像URL提取用户信息
                                    if (!userUrl) {
                                        const avatarElement = element.querySelector('img, [class*="avatar"]');
                                    ...
                                    // 创建弹幕唯一标识（只基于昵称和内容，去掉时间戳）
                                    const danmakuKey = `${nickname}:${content}`;
                                    ...
                                    if (window.danmakuCache.has(danmakuKey)) {
                                        console.log(`❌ 重复弹幕，跳过: ${danmakuKey}`);
                                        return;
                                    }
                                    
                                    // 添加到缓存，设置过期时间
                                    window.danmakuCache.add(danmakuKey);
                                    setTimeout(() => window.danmakuCache.delete(danmakuKey), 30000);
            ...
            }, keywords, Object.fromEntries(this.userIdToUrlMap));
