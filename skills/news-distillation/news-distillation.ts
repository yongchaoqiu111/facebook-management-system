import { LLMSlient } from '../../core/llm/llm-client';
...
interface MediaLibrary {
  reputableMedia: MediaInfo[];
}

interface DistilledNews {
  title: string;
  summary: string;
  keyPoints: string[];
...
export class NewsDistillationSkill {
  private llmClient: LLMSlient;
  private mediaLibrary: MediaLibrary;
  private lastImageIndex: number = 0;
  private imageIndexFile: string;

  constructor() {
    this.llmClient = new LLMSlient();
    this.mediaLibrary = this.loadMediaLibrary();
    // 初始化图片索引文件路径
    if (__dirname.includes('dist')) {
      this.imageIndexFile = path.join(__dirname, '../../../data/image-index.json');
    } else {
      this.imageIndexFile = path.join(__dirname, '../../data/image-index.json');
    }
    // 加载上次的图片索引
  ...
  private loadLastImageIndex(): void {
    try {
      if (fs.existsSync(this.imageIndexFile)) {
        const content = fs.readFileSync(this.imageIndexFile, 'utf8');
  ...
  private saveLastImageIndex(): void {
    try {
      const data = { lastIndex: this.lastImageIndex };
      fs.writeFileSync(this.imageIndexFile, JSON.stringify(data, null, 2), 'utf8');
    } catch (error) {
      console.error(`[${MODULE}] 保存图片索引失败:`, error);
    }
  }

  private getNextImage(): string | null {
    try {
  ...
  private loadMediaLibrary(): MediaLibrary {
    try {
      // 构建正确的媒体库文件路径
      let mediaLibraryPath;
      if (__dirname.includes('dist')) {
        // 编译后的路径
        mediaLibraryPath = path.join(__dirname, '../../../data/media-library.json');
      } else {
        // 源代码路径
        mediaLibraryPath = path.join(__dirname, '../../data/media-library.json');
      }
      ...
      if (fs.existsSync(mediaLibraryPath)) {
        const content = fs.readFileSync(mediaLibraryPath, 'utf8');
  ...
  async execute(traceId: string): Promise<any> {
    try {
  ...
  private async fetchNewsFromPlatform(platform: string, keyword: string): Promise<NewsArticle[]> {
    let url = '';
    let apiKey = '';
  ...
  private async distillNews(articles: NewsArticle[], traceId: string): Promise<DistilledNews> {
    try {
      // 构建提示词，使用更简单的格式
...
`;
      ...
      if (analysisMatch) refinedContent = analysisMatch[1].trim();
  ...
  private getMockDistilledNews(): DistilledNews {
    return {
      title: 'AI 领域最新动态分析',
  ...
  private formatNews(distilled: DistilledNews): string {
    // 格式化新闻，添加 emoji 和分隔线
  ...
  private async saveResult(formattedNews: string): Promise<void> {
    const fs = require('fs');
...
export const newsDistillationSkill = new NewsDistillationSkill();
