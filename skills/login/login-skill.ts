...
export interface LoginInput {
  ...
  browserInstances?: { [key: string]: any };
}
...
async function saveCookie(page: any, platform: 'facebook' | 'tiktok'): Promise<string> {
  ...
  try {
    ...
    // 生成cookie文件名
    const cookieFileName = `${platform}_${Date.now()}.txt`;
...
export async function loginSkill(input: LoginInput): Promise<LoginOutput> {
  ...
  try {
    ...
    // 使用传入的浏览器实例对象
    const instances = input.browserInstances || browserInstances;
    ...
    if (input.action === 'login') {
      ...
      // 保存浏览器实例
      instances[input.platform] = result;
      ...
      // 保存当前浏览器的cookie
      if (instances[input.platform]) {
        result = instances[input.platform];
      ...
      // 清除浏览器实例
      delete instances[input.platform];
