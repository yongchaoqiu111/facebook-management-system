...
async function postToFacebookReal(input) {
  console.log('启动 Facebook 发帖脚本...');
  ...
  try {
      ...
      if (fileInput) {
        for (const imagePath of input.imagePaths) {
          ...
          if (fs.existsSync(absolutePath)) {
            console.log(`尝试上传图片: ${absolutePath}`);
    ...
    try {
      ...
      await textArea.type(input.text);
    ...
    // 保持浏览器打开，让用户查看发布结果
    console.log('浏览器窗口已保持打开，您可以查看发布结果');
    // await browser.close();
...
module.exports = { postToFacebookReal };
