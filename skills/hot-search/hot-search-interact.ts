...
export interface HotSearchInteractInput {
  skillId: string;
  traceId: string;
}

export interface HotSearchInteractOutput {
  code: number;
  data: {
...
function readNews(): string[] {
  const newsFile = path.join(__dirname, '../../user-config/assets/news/1.txt');
  try {
...
async function generateAIExpertPost(newsContent: string): Promise<string> {
  // 读取配置文件中的提示词
...
function savePost(post: string): boolean {
...
export async function interactHotSearch(input: HotSearchInteractInput): Promise<HotSearchInteractOutput> {
  log('开始热搜词二次交互任务');
